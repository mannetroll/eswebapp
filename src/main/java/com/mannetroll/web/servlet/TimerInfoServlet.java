package com.mannetroll.web.servlet;
...
public class TimerInfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("freq") != null) {
			header(NAME + " - frequency - " + datestr, false, out, apikey);
			...
			header(NAME + " - memory - " + datestr, true, out, apikey);
			...
			header(NAME + " - payload - " + datestr, false, out, apikey);
			...
			header(NAME + " - performance - " + datestr, reload, out, apikey);
			baseurl = "?stat=performance";
			if (apikey != null) {
				baseurl += "&apikey=" + apikey;
			}
			statisticsTable(out, tistat, sort, baseurl);
	 ...
	 * @throws IOException
	 */
	private void header(String title, boolean reload, ServletOutputStream out, String apikey) throws IOException {
		...
		out.println("</head>");
		out.println("<body style='margin-left: 25px;margin-right: 25px'>");
		...
		String jvm = shorten(System.getProperty("java.vm.name")) + ", " + System.getProperty("java.vm.version");
		...
		if (apikey != null) {
			out.println("<input type=\"hidden\" name=\"apikey\" value=\"" + apikey + "\"/>");
		}
		out.println("</form></td>");
		out.println("</tr></table>");
	}

	private String shorten(String property) {
		if (property != null) {
			int length = property.length();
			if (length > 24) {
				return property.substring(0, 25);
			}
		}
		return property;
	}
	 ...
	 * @throws IOException
	 */
	private void statisticsTable(ServletOutputStream out, TimerInfoStats stats, String sort, String baseurl)
			throws IOException {
		...
		for (TimerInfoItem item : treeSet) {
			...
			if (statDataArray[0] != 0) {
				...
				if (key.startsWith("200") || key.equals("TOTAL")) {
